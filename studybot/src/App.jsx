...
export default function App(){
  ...
  async function handleClick(i,title){
    setActiveIdx(i);setLoading(true);setAnswer('');setError('');setCurrentTitle(title);
    ...
    try{
      ...
      if(e.message.includes('API_KEY_INVALID')||e.message.includes('401')){
        localStorage.removeItem('gemini_key');setApiKey('');
      }
  ...
  if(page==='home')return(
    <div style={{minHeight:'100vh',background:'#0d1117',color:'#e2e8f0',fontFamily:"'Noto Sans KR',system-ui,sans-serif",overflowY:'auto'}}>
      ...
      <div style={{textAlign:'center',padding:'80px 24px 60px'}}>
        ...
        <p style={{fontSize:15,color:'#8b949e',maxWidth:480,margin:'0 auto 36px',lineHeight:1.7}}>12개 과목 · 200개 이상의 학습 주제<br/>클릭 한 번으로 Gemini AI가 바로 설명해드려요</p>
    ...
    <div style={{height:'100vh',display:'flex',flexDirection:'column',background:'#0d1117',color:'#e2e8f0',fontFamily:"'Noto Sans KR',system-ui,sans-serif",overflow:'hidden',fontSize:13}}>
      <div style={{background:'#161b22',borderBottom:'1px solid #21262d',padding:'7px 12px',display:'flex',alignItems:'center',gap:5,overflowX:'auto',flexShrink:0,scrollbarWidth:'none'}}>
        ...
        {GROUPS.map(g=>(
          <span key={g.label} style={{display:'flex',alignItems:'center',gap:4}}>
            ...
            {g.keys.map(k=>(
              <button key={k} onClick={()=>{setSubj(k);setActiveIdx(null);setAnswer('');}}
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        <div style={{width:220,flexShrink:0,overflowY:'auto',borderRight:'1px solid #21262d',padding:'8px'}}>
          ...
          {items.map((title,i)=>(
            <div key={i} onClick={()=>handleClick(i,title)}
              style={{padding:'9px 10px',borderRadius:7,marginBottom:3,cursor:'pointer',border:'1px solid',borderColor:activeIdx===i?'#58a6ff':'#21262d',background:activeIdx===i?'#1f3354':'#161b22',display:'flex',gap:8,alignItems:'flex-start'}}
              onMouseEnter={e=>{if(activeIdx!==i){e.currentTarget.style.borderColor='#30363d';e.currentTarget.style.background='#1c2128';}}}
              onMouseLeave={e=>{if(activeIdx!==i){e.currentTarget.style.borderColor='#21262d';e.currentTarget.style.background='#161b22';}}}>
              <span style={{color:'#484f58',fontSize:10,fontFamily:'monospace',minWidth:18,flexShrink:0,marginTop:2}}>{i+1}.</span>
              <span style={{color:activeIdx===i?'#58a6ff':'#c9d1d9',fontSize:12,lineHeight:1.45}}>{title}</span>
            </div>
          ))}
        </div>
            ...
            <div>
              ...
              {activeIdx!==null&&activeIdx<items.length-1&&(
                <div style={{marginTop:24,paddingTop:16,borderTop:'1px solid #21262d'}}>
                  <button onClick={()=>handleClick(activeIdx+1,items[activeIdx+1])} style={{padding:'8px 16px',borderRadius:8,border:'1px solid #30363d',background:'#161b22',color:'#c9d1d9',fontSize:12,cursor:'pointer'}}>
