...
const TOC = {
  ...
  english_high: ['시제 완전 정복','조동자 심화','분사와 분사구문','관계사 완전 정복','가정법','특수구문 강조·도치','어휘 확장 전략','독해 전략','수능 문법 포인트','에세이·서술형 작문'],
...
export default function App() {
  ...
  function click(i, title) {
    ...
    navigator.clipboard.writeText(msg)
      .then(() => setSent('📋 복사완료! Claude 채팅창에 Ctrl+V → Enter'))
      .catch(() => setSent('👉 ' + msg));
  }
  ...
  if (page === 'home') return (
    <div style={{minHeight:'100vh',background:'#0d1117',color:'#e2e8f0',fontFamily:"'Noto Sans KR', system-ui, sans-serif",overflowY:'auto'}}>
      <nav style={{position:'sticky',top:0,zIndex:100,background:'rgba(13,17,23,0.9)',backdropFilter:'blur(12px)',borderBottom:'1px solid #21262d',padding:'0 24px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <button onClick={()=>setPage('app')} style={{padding:'7px 18px',borderRadius:8,border:'none',background:'#58a6ff',color:'#000',fontSize:13,fontWeight:700,cursor:'pointer'}}>
          앱 시작하기 →
        </button>
      </nav>
      ...
      <div style={{textAlign:'center',padding:'80px 24px 60px',position:'relative'}}>
        ...
        <div style={{position:'relative'}}>
          ...
          <p style={{fontSize:15,color:'#8b949e',maxWidth:480,margin:'0 auto 36px',lineHeight:1.7}}>
            12개 과목 · 200개 이상의 학습 주제<br/>클릭 한 번으로 Claude에게 질문 전달
          </p>
          <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
            <button onClick={()=>setPage('app')} style={{padding:'12px 28px',borderRadius:10,border:'none',background:'#58a6ff',color:'#000',fontSize:15,fontWeight:700,cursor:'pointer'}}>
              📚 지금 바로 시작하기
            </button>
          </div>
        </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',background:'#0d1117',borderTop:'1px solid #21262d',borderBottom:'1px solid #21262d',margin:'0 0 56px'}}>
        {STATS.map(({num,label})=>(
          <div key={label} style={{padding:'28px 16px',textAlign:'center',borderRight:'1px solid #21262d'}}>
            <div style={{fontSize:26,fontWeight:900,color:'#58a6ff'}}>{num}</div>
      ...
      <div style={{padding:'0 24px 56px',maxWidth:840,margin:'0 auto'}}>
        <h2 style={{textAlign:'center',fontSize:20,fontWeight:700,marginBottom:32}}>사용 방법</h2>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:16}}>
          {[
            {step:'01',title:'과목 & 난이도 선택',desc:'원하는 과목과 학습 난이도를 선택하세요'},
            ...
            <div key={step} style={{background:'#161b22',border:'1px solid #21262d',borderRadius:12,padding:'24px 20px',position:'relative',overflow:'hidden'}}>
              <div style={{position:'absolute',top:12,right:16,fontSize:30,fontWeight:900,color:'#21262d'}}>{step}</div>
              <div style={{fontSize:12,color:'#58a6ff',fontWeight:700,marginBottom:8}}>STEP {step}</div>
              <div style={{fontWeight:700,fontSize:14,marginBottom:8}}>{title}</div>
              <div style={{fontSize:13,color:'#8b949e',lineHeight:1.6}}>{desc}</div>
            </div>
          ))}
      ...
      <div style={{textAlign:'center',padding:'48px 24px 64px',borderTop:'1px solid #21262d'}}>
        ...
        <button onClick={()=>setPage('app')} style={{padding:'14px 36px',borderRadius:10,border:'none',background:'#58a6ff',color:'#000',fontSize:16,fontWeight:700,cursor:'pointer'}}>
          📚 StudyBot 열기
        </button>
      </div>
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',background:'#0d1117',color:'#e2e8f0',fontFamily:"'Noto Sans KR', system-ui, sans-serif",overflow:'hidden',fontSize:13}}>
      <div style={{background:'#161b22',borderBottom:'1px solid #21262d',padding:'7px 12px',display:'flex',alignItems:'center',gap:5,overflowX:'auto',flexShrink:0,scrollbarWidth:'none'}}>
        <button onClick={()=>{setPage('home');setSent('');}} style={{padding:'3px 10px',borderRadius:16,border:'1px solid #21262d',background:'transparent',color:'#8b949e',fontSize:11,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>← 홈</button>
      ...
      {subj==='math' && (
        <div style={{background:'#161b22',borderBottom:'1px solid #21262d',padding:'6px 12px',display:'flex',gap:6,flexShrink:0}}>
          {MATH_S.map(({k,l,c})=>(<button key={k} onClick={()=>setSub(p=>({...p,math:k}))} style={{flex:1,padding:'4px',borderRadius:8,border:'1px solid',borderColor:sub.math===k?c:'#21262d',background:sub.math===k?c+'33':'transparent',color:sub.math===k?c:'#484f58',fontSize:12,fontWeight:sub.math===k?700:400,cursor:'pointer'}}>{l}</button>))}
        </div>
      )}
      {subj==='english' && (
        <div style={{background:'#161b22',borderBottom:'1px solid #21262d',padding:'6px 12px',display:'flex',gap:6,flexShrink:0}}>
          {ENG_S.map(({k,l,c})=>(<button key={k} onClick={()=>setSub(p=>({...p,english:k}))} style={{flex:1,padding:'4px',borderRadius:8,border:'1px solid',borderColor:sub.english===k?c:'#21262d',background:sub.english===k?c+'33':'transparent',color:sub.english===k?c:'#484f58',fontSize:12,fontWeight:sub.english===k?700:400,cursor:'pointer'}}>{l}</button>))}
        </div>
      )}
      {subj==='science' && (
        <div style={{background:'#161b22',borderBottom:'1px solid #21262d',padding:'6px 12px',display:'flex',flexWrap:'wrap',gap:5,flexShrink:0}}>
          {SCI_S.map(({k,l,c})=>(<button key={k} onClick={()=>setSub(p=>({...p,science:k}))} style={{flex:'1 1 calc(33% - 5px)',padding:'4px',borderRadius:8,border:'1px solid',borderColor:sub.science===k?c:'#21262d',background:sub.science===k?c+'33':'transparent',color:sub.science===k?c:'#484f58',fontSize:11.5,fontWeight:sub.science===k?700:400,cursor:'pointer',textAlign:'center'}}>{l}</button>))}
        </div>
      )}
      ...
      <div style={{background:'#161b22',borderTop:'1px solid #21262d',padding:'8px 12px',fontSize:12,color:sent?'#3fb950':'#484f58',textAlign:'center',flexShrink:0,fontWeight:sent?600:400}}>
        {sent || '목차 클릭 → 자동 복사 → Claude 채팅창에 Ctrl+V → Enter'}
      </div>
    </div>
  );
}
